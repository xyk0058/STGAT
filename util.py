...
class NetDataSet(data.Dataset):
    def __init__(self, x, y, isTrainSet=False):
        ...
        if isTrainSet:
            ...
            self.x_ = self.x[:, :, idx, :]
        # self.x_ = np.random.permutation(self.x)

    def __getitem__(self, index):
        if self.isTrainSet:
            return self.x[index].transpose(1, 0, 2), self.y[index].transpose(2, 1, 0), self.x_[index].transpose(1, 0, 2)
        else:
            return self.x[index].transpose(1, 0, 2), self.y[index].transpose(2, 1, 0)
...
def DataLoader(x, y, batch_size, isTrainSet=False, shuffle=True):
    dataset = NetDataSet(x, y, isTrainSet)
...
def load_adj(pkl_filename, adjtype):
    ...
    # print(adj_mx)
    # adj = [adj_mx.astype(np.float32).todense()]
    adj = [adj_mx]
...
def load_dataset(dataset_dir, batch_size, valid_batch_size= None, test_batch_size=None):
    ...
    data['train_loader'] = DataLoader(data['x_train'], data['y_train'], batch_size, isTrainSet=True)
